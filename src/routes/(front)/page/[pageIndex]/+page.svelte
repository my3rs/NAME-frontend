...
<main class="flex-1 p-6 overflow-y-auto h-screen">
    <div class="pl-2 pr-2">
        {#each posts as post}
            <article class="overflow-hidden border-solid border-b border-gray-200">
                <div class="p-6">

                    <h2 class="text-xl font-bold mb-2">
                        <a href="/posts/{post.id}" target="_blank" class="text-slate-900 hover:text-blue-800">{post.title}</a>
                    </h2>

                    <!-- 文章摘要 -->
                    <!-- 如果abstract字段长度为0，提取除掉图片标记的text前120字作为摘要 -->
                    <p class="text-gray-600 mb-4 font-normal text-gray-400 leading-relaxed">
                    {#if post.abstract && post.abstract.length > 0}
                        <p class="text-gray-600 mb-4">{post.abstract}</p>
                    {:else}
                        <p class="text-gray-600 mb-4">
                            {(() => {
                                let text = post.text.replace(/!\[.*?\]\(.*?\)/g, '');
    ...
    <div class="flex  mt-6 ">
        <Pagination.Root {count} {perPage} {siblingCount} let:pages let:currentPage  onPageChange={onPageChange} class="items-center">
            <Pagination.Content>
                <Pagination.Item>
